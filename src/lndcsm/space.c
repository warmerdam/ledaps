...
#define SPACE_PIXEL_SIZE_UNITS_HDF ("PixelSizeUnits")
...
bool PutSpaceDefHDF(Space_def_t *this, char *file_name, int nsds, 
  ...
  char *cproj;
  double dval[1]; /* Feng changes from 0 to 1 in 4/17/2009 */
  char *ctype;
  int it;
  int32 hdf_id;
  int32 vgroup_id[3];
...
bool GetSpaceDefHDF(Space_def_t *this, char *file_name, char *grid_name) {
  ...
  sds_file_id = SDstart((char *)file_name, DFACC_RDONLY);
  if (sds_file_id == HDF_ERROR) 
...
bool PutGeoDefHDF(Geo_def_t *this, char *file_name, int nsds, 
  ...
  int32 vgroup_id[3];
  int32 sds_index, sds_id;
  char b1[20],b2[20],b3[20],b4[20]; /* temp fmt buffers */

  /* Check inputs */

  if (nsds <= 0) 
    RETURN_ERROR("invalid number of SDSs", "PutGeoDefHDF", false);
  for (isds = 0; isds < nsds; isds++)
    if (strlen(sds_names[isds]) < 1)  
      RETURN_ERROR("invalid SDS name", "PutGeoDefHDF", false);
  if (strlen(grid_name) < 1)  
    RETURN_ERROR("invalid grid name [0]", "PutGeoDefHDF", false);
  ...
  if (!AppendMeta(struct_meta, &ic, cbuf))
    RETURN_ERROR("appending to metadata string (head)", "PutGeoDefHDF", 
                  false);
  ...
  sprintf(cbuf, 
    ...
    "\t\tXDim=%d\n" 
    "\t\tYDim=%d\n" 
    "\t\tUpperLeftPointMtrs=(%s,%s)\n" 
    "\t\tLowerRightMtrs=(%s,%s)\n"
    "\t\tProjection=GEO\n", 
    grid_name, 
    this->img_size.s, this->img_size.l, 
    deg2dms0(this->ul_corner.lon*DEG,b1), deg2dms0(this->ul_corner.lat*DEG,b2),
    deg2dms0(this->lr_corner.lon*DEG,b3), deg2dms0(this->lr_corner.lat*DEG,b4));
  ...
  for (isds = 0; isds < nsds; isds++) {
    ...
    if (ctype == (char *)NULL)
      RETURN_ERROR("getting hdf type name string", "PutGeoDefHDF", false);
  ...
  if (!AppendMeta(struct_meta, &ic, cbuf))
    RETURN_ERROR("appending to metadata string (tail)", "PutGeoDefHDF", 
                  false);
  ...
  sds_file_id = SDstart((char *)file_name, DFACC_RDWR);
  if (sds_file_id == HDF_ERROR) 
    RETURN_ERROR("opening output file for SD access (1)", "PutGeoDefHDF", 
                 false); 
  
  attr.type = DFNT_FLOAT64;
  attr.nval = 2;
  attr.name = SPACE_PIXEL_SIZE_HDF;
  dval[0] = (double)this->geo_pixel_size_x;
  dval[1] = (double)this->geo_pixel_size_y;
  ...
  attr.type = DFNT_CHAR8;
  attr.nval = strlen(pixelsize_units);
  attr.name = SPACE_PIXEL_SIZE_UNITS_HDF;
  if (!PutAttrString(sds_file_id, &attr, pixelsize_units))
    RETURN_ERROR("writing attribute (pixelsize_units)", "PutGeoDefHDF", false);

  attr.type = DFNT_CHAR8;
  attr.nval = strlen(hdfeos_version);
  attr.name = SPACE_HDFEOS_VERSION;
  if (!PutAttrString(sds_file_id, &attr, hdfeos_version))
    RETURN_ERROR("writing attribute (hdfeos_version)", "PutGeoDefHDF", false);

  attr.type = DFNT_CHAR8;
  attr.nval = strlen(struct_meta);
  attr.name = SPACE_STRUCT_METADATA;
  if (!PutAttrString(sds_file_id, &attr, struct_meta))
    RETURN_ERROR("writing attribute (struct_meta)", "PutGeoDefHDF", false);
  ...
  if (SDend(sds_file_id) == HDF_ERROR) 
    RETURN_ERROR("ending SD access (1)", "PutGeoDefHDF", false);

  /* Setup the HDF Vgroup */

  hdf_id = Hopen((char *)file_name, DFACC_RDWR, 0);
  if (hdf_id == HDF_ERROR) 
    RETURN_ERROR("opening HDF file (Hopen)", "PutGeoDefHDF", false);

  /* Start the Vgroup access */

  if (Vstart(hdf_id) == HDF_ERROR) 
    RETURN_ERROR("starting Vgroup access (Vstart)", "PutGeoDefHDF", false);
  ...
  for (isds = 0; isds < nsds; isds++) {
    ...
    sds_id = SDselect(sds_file_id, sds_index);
    if (sds_id == HDF_ERROR) 
      RETURN_ERROR("getting SDS ID (SDselect)", "PutGeoDefHDF", false);
    if (Vaddtagref(vgroup_id[1], DFTAG_NDG, SDidtoref(sds_id)) == 
        HDF_ERROR) 
      RETURN_ERROR("adding reference tag to SDS (Vaddtagref)", 
                   "PutGeoDefHDF", false);
    if (SDendaccess(sds_id) == HDF_ERROR) 
      RETURN_ERROR("ending access to SDS", "PutGeoDefHDF", false);
  }
  
  if (SDend(sds_file_id) == HDF_ERROR) 
    RETURN_ERROR("ending SD access (2)", "PutGeoDefHDF", false);
  ...
  if (Vend(hdf_id) == HDF_ERROR) 
    RETURN_ERROR("end Vgroup access (Vend)", "PutGeoDefHDF", false);
  if (Hclose(hdf_id) == HDF_ERROR) 
    RETURN_ERROR("end HDF access (Hclose)", "PutGeoDefHDF", false);
